...
{
    private EntityFX fx;  //定义了 EntityFX fx 之后 要在 Start 里获取组件 fx = GetComponent<EntityFX>(); 才能在这个脚本中使用 EntityFX 里的方法

    [Header("Major stats")]
    public Stat strength; // 
    public Stat agility;
    public Stat intelligence; // 一点智力 + 1 魔法伤害 + 3 魔抗
    public Stat vitality; // 一点生命力 + 5 最大生命上限
    ...
    public bool isChilled; //减速 + 减护甲
    public bool isShocked; //减命中
    ...
    public virtual void DoDamage(CharacterStats _targetStats)
    {
        ...
        // 这个 transform 是伤害检测的位置
        // 也就是 调用 DoDamage() 方法时的位置
    ...
    private void AttemptToApplyAilments(CharacterStats _targetStats, int _fireDamage, int _iceDamage, int _lightningDamage)
    {
        ...
        {
            if (Random.value < .3f && _fireDamage > 0) // Random.value 生成 0 到 1 之间的一个随机浮点数
            {
                canApplyIgnite = true;
    ...
    public void ApplyAilments(bool _ignite, bool _chill, bool _shock)
    {
        ...
        if (_ignite && canApplyIgnite) // _ignite 和 canApplyIgnite 为 true 才能进入判断语句
        {
            isIgnited = _ignite;
    ...
    private void HitNearestTargetWithShockStrike()
    {
        ...
        {
            if (hit.GetComponent<Enemy>() != null && Vector2.Distance(transform.position, hit.transform.position) > 1) // 检测离该敌人最近的敌人的位置
            {
                float distenceToEnemy = Vector2.Distance(transform.position, hit.transform.position);

                if (distenceToEnemy < closestDistance)
                {
    ...
    protected int CalculateCriticalDamage(int _damage)
    {
        ...
        return Mathf.RoundToInt(critDamage);  //对 critDamage 四舍五入取整 
    }
