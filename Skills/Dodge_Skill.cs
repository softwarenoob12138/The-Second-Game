...
public class Dodge_Skill : Skill
{
    ...
    {
        if(unlockDodgeButton.unlocked && !dodgeUnlocked) // 确保 它只能在被解锁的时候 调用一次
        {
    ...
    public void CreateMirageOnDodge()
    {
        ...
        {
            SkillManager.instance.clone.CreateClone(player.transform, new Vector3(2 * player.facingDir, .4f)); 
        }
    }
}
